...
import {MEMOH_STEERING_METHOD, MemohSteering, parseMemohSteerRequest} from "./MemohSteering";
...
export class CodexAcpServer {
    private readonly memohSteering = new Map<string, MemohSteering>();
    ...
    async extMethod(method: string, params: Record<string, unknown>): Promise<Record<string, unknown>> {
        if (method === MEMOH_STEERING_METHOD) {
            const request = parseMemohSteerRequest(params);
            const scope = this.memohSteering.get(request.sessionId);
            if (!scope) throw RequestError.invalidParams("No active Memoh run for this session");
            return await scope.steer(request);
        }
    ...
    async prompt(
        ...
        const runId = params._meta?.["memoh/runId"];
        const memohSteering = typeof runId === "string" && runId.trim()
            ? new MemohSteering(params.sessionId, runId, activePrompt.signal,
                () => this.promptShouldStop(params.sessionId, activePrompt) ? null : sessionState.currentTurnId,
                (turnId, id, text) => this.runWithProcessCheck(() => this.codexAcpClient.steerTurn({
                    threadId: params.sessionId, turnId, clientUserMessageId: id,
                    prompt: [{type: "text", text}],
                })),
                (id, text) => this.connection.notify(acp.methods.client.session.update, {
                    sessionId: params.sessionId,
                    update: createUserMessageChunk({type: "text", text}, id),
                }))
            : null;
        this.memohSteering.get(params.sessionId)?.finish();
        this.memohSteering.delete(params.sessionId);
        if (memohSteering) this.memohSteering.set(params.sessionId, memohSteering);
        ...
        try {
            ...
            await this.codexAcpClient.subscribeToSessionEvents(params.sessionId,
                async (event) => {
                    await memohSteering?.handleNotification(event);
            ...
            memohSteering?.finish();
            if (this.memohSteering.get(params.sessionId) === memohSteering) this.memohSteering.delete(params.sessionId);
