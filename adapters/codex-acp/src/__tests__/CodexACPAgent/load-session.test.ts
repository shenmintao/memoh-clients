...
describe("CodexACPAgent - loadSession", () => {
    ...
    it("should recover response item function calls when app-server history omits tool items", async () => {
        ...
        try {
            ...
            // Normalize the platform-native fixture path for the shared snapshot.
            const dump = fixture.getAcpConnectionDump([]).replaceAll(
                JSON.stringify(join("/test/project", "src/index.ts")).slice(1, -1),
