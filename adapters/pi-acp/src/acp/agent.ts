...
export class PiAcpAgent implements ACPAgent {
  ...
  async newSession(params: NewSessionRequest) {
    ...
    // Forward session-scoped MCP servers to Pi's bundled extension.
  ...
  async extMethod(method: string, params: Record<string, unknown>): Promise<Record<string, unknown>> {
    if (method !== '_memoh/steer') throw RequestError.methodNotFound(method)
    if (
      ...
      throw RequestError.invalidParams('sessionId, runId, id and text are required')
    }
    const session = this.sessions.get(params.sessionId)
    if (!session) throw RequestError.invalidParams('Session is not active')
    return session.steer(params.runId, params.id, params.text)
  }
  ...
  async prompt(params: PromptRequest): Promise<PromptResponse> {
    ...
    const result = await session.prompt(
      message,
      images,
      typeof params._meta?.['memoh/runId'] === 'string' ? params._meta['memoh/runId'] : ''
    )
