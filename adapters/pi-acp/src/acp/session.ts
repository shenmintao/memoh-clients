...
export class PiAcpSession {
  ...
  private steeringCleanupFailed = false
  private memohRunId = ''
  private pendingSteer: { text: string; resolve: (value: { applied: boolean }) => void } | null = null
  private steeringReceipts = new Map<string, { text: string; result: Promise<{ applied: boolean }> }>()
  ...
  async prompt(message: string, images: unknown[] = [], runId = ''): Promise<StopReason> {
    if (this.steeringCleanupFailed) throw new Error('Steering cleanup failed; create a fresh session')
  ...
  steer(runId: string, id: string, text: string): Promise<{ applied: boolean }> {
    if (!runId || runId !== this.memohRunId || !this.pendingTurn || !this.inAgentLoop || this.cancelRequested) {
      throw RequestError.invalidParams('The target turn is not running')
    }
    const previous = this.steeringReceipts.get(id)
    if (previous) {
      if (previous.text !== text) throw RequestError.invalidParams('Conflicting steering ID')
      return previous.result
    }
    if (!id || !text.trim() || text.length > 32000 || this.pendingSteer || this.steeringReceipts.size >= 128) {
      throw RequestError.invalidParams('Invalid or busy steering request')
    }
    const result = new Promise<{ applied: boolean }>((resolve, reject) => {
      const pending = { text, resolve }
      this.pendingSteer = pending
      void this.proc
        .steer(text)
        .then(accepted => {
          if (!accepted && this.pendingSteer === pending) {
        ...
        .catch(() => {
          if (this.pendingSteer === pending) {
            this.pendingSteer = null
            this.steeringCleanupFailed = true
            reject(new Error('Steering delivery is unconfirmed'))
          }
        })
    })
    this.steeringReceipts.set(id, { text, result })
    return result
  }
  ...
  async cancel(): Promise<void> {
    ...
    if (this.pendingSteer) {
      ...
      this.pendingSteer.resolve({ applied: false })
      this.pendingSteer = null
    }
  }
  ...
  private handlePiEvent(ev: PiRpcEvent) {
    ...
    switch (type) {
      case 'message_start': {
        const message = ev.message as { role?: string; content?: Array<{ type?: string; text?: string }> } | undefined
        const text = Array.isArray(message?.content)
          ? message.content
              ...
              .join('\n')
          : ''
        if (message?.role === 'user' && this.pendingSteer?.text === text) {
          const pending = this.pendingSteer
          this.pendingSteer = null
          this.emit({ sessionUpdate: 'user_message_chunk', content: { type: 'text', text } })
          void this.flushEmits().then(() => pending.resolve({ applied: true }))
        }
        break
      }
      ...
      case 'agent_settled': {
        const unconsumed = this.pendingSteer
        this.pendingSteer = null
        this.memohRunId = ''
        const clearPending = unconsumed
          ? this.proc.clearQueue().then(() => {
              unconsumed.resolve({ applied: false })
            })
          : Promise.resolve()
        ...
        void clearPending
          ...
          .catch(() => {
            this.steeringCleanupFailed = true
            unconsumed?.resolve({ applied: false })
            this.pendingTurn?.reject(new Error('Steering cleanup failed'))
            this.pendingTurn = null
          })
        break
      }
