...
export class PiRpcProcess {
  ...
  static async spawn(params: SpawnParams): Promise<PiRpcProcess> {
    ...
    const hasMcp = Boolean(params.mcpServers?.length)
    let mcpDirectory: string | undefined
    let mcpConfig: string | undefined
    const cleanupMcp = () => {
      if (mcpDirectory) rmSync(mcpDirectory, { recursive: true, force: true })
      mcpDirectory = undefined
    }
    if (hasMcp) {
      const bundled = fileURLToPath(new URL('./mcp-extension.js', import.meta.url))
      args.push(
        '--extension',
        existsSync(bundled) ? bundled : fileURLToPath(new URL('../mcp-extension.ts', import.meta.url))
      )
      mcpDirectory = mkdtempSync(join(tmpdir(), 'pi-acp-mcp-'))
      mcpConfig = join(mcpDirectory, 'servers.json')
      writeFileSync(mcpConfig, JSON.stringify(params.mcpServers), { mode: 0o600 })
    }
    ...
    const child = spawn(cmd, args, {
      cwd: params.cwd,
      stdio: 'pipe',
      env: { ...process.env, ...(mcpConfig ? { PI_ACP_MCP_CONFIG: mcpConfig } : {}) },
      windowsHide: true,
      shell: shouldUseShellForPiCommand(cmd)
    }) as ChildProcessWithoutNullStreams
    child.once('exit', cleanupMcp)
    child.once('error', cleanupMcp)
  ...
  async clearQueue(): Promise<void> {
    ...
    if (!res.success) throw new Error('Pi could not clear queued messages')
  }
